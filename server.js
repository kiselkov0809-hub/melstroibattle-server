...
function handleMessage(ws, data) {
    ...
    if (data.type === 'move') {
        for (let roomId in rooms) {
            ...
            if (room.p1 === ws) {
                room.p2.send(JSON.stringify({ type: 'opponent_move', x: data.x }));
            }
            if (room.p2 === ws) {
                room.p1.send(JSON.stringify({ type: 'opponent_move', x: data.x }));
            }
        }
    }
    ...
    if (data.type === 'attack') {
        for (let roomId in rooms) {
            const room = rooms[roomId];
            if (room.p1 === ws) {
                room.p2.send(JSON.stringify({ type: 'opponent_attack', dmg: data.dmg }));
            }
            if (room.p2 === ws) {
                room.p1.send(JSON.stringify({ type: 'opponent_attack', dmg: data.dmg }));
            }
        }
    }
