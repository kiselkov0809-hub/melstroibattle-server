...
function handleMessage(ws, data) {
    switch(data.type) {
        case 'join_queue':
            matchmaking.push({ ws, nick: data.nick });
        ...
        case 'attack':
            ...
            if (room2) {
                ...
                if (opponent) {
                    opponent.send(JSON.stringify({ type: 'opponent_attack', dmg: data.dmg, hp: data.hp }));
                }
            }
            break;
        case 'game_over':
            ...
            if (room3) {
                ...
                if (opponent) {
                    opponent.send(JSON.stringify({ type: 'game_result', win: data.win }));
                }
...
function findMatch(ws) {
    if (matchmaking.length >= 2) {
        ...
        const room = { id: roomId, p1: p1.ws, p2: p2.ws, nick1: p1.nick, nick2: p2.nick };
        rooms[roomId] = room;
        p1.ws.send(JSON.stringify({ type: 'match_found', opponent: p2.nick, side: 'left', roomId }));
        p2.ws.send(JSON.stringify({ type: 'match_found', opponent: p1.nick, side: 'right', roomId }));
    }
}
